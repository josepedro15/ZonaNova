'use server';
import{revalidatePath}from'next/cache';import{criarClienteServidor}from'@/lib/supabase/server';import{criarClienteAdmin}from'@/lib/supabase/admin';import{dataValida,janelaDoDia}from'@/lib/analise';import{conversasDoFechamento}from'@/lib/fechamento';import{desligarWhatsappDe}from'@/lib/desligar';
...
export async function reprocessarDia(form:FormData){const dataRef=String(form.get('data')??'');const ctx=await operador();if(!ctx||ctx.role!=='admin'||!dataValida(dataRef))return;const{inicio,fim}=janelaDoDia(dataRef);const ids=await conversasDoFechamento(ctx.admin,inicio,fim);if(ids.length){await ctx.admin.from('fila_processamento').delete().eq('data_ref',dataRef).neq('tipo','transcricao');await ctx.admin.from('aderencia_conversa').delete().eq('data_ref',dataRef);await ctx.admin.from('mec_observacoes').delete().eq('data_ref',dataRef);await ctx.admin.from('aderencia_diaria').delete().eq('data_ref',dataRef);await ctx.admin.from('analises_conversa').delete().eq('data_ref',dataRef);await ctx.admin.from('relatorios_diarios').delete().eq('data_ref',dataRef);await ctx.admin.from('relatorios_unidade').delete().eq('data_ref',dataRef);await ctx.admin.from('relatorios_rede').delete().eq('data_ref',dataRef);await ctx.admin.from('fila_processamento').insert(ids.map(id=>({tipo:'analise_conversa',referencia_id:id,data_ref:dataRef})));}await ctx.admin.from('eventos_admin').insert({actor_id:ctx.user.id,acao:'reprocessou_dia',detalhes:{data_ref:dataRef,conversas:ids.length}});revalidatePath('/admin')}
