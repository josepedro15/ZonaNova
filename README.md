...
npm run test:rls               # Postgres local: migrations + seed + 35 testes de RLS
                               # macOS precisa de: brew install postgresql@17
```

## Estado atual

**Fase 3 — Fundação concluída, e já em Supabase real.** Auth, papéis e
isolamento por unidade de pé e testados (`tests/rls.sql`, 32 asserções mais 3
guardas de privilégio — 35 de 35 a passar). As 22 telas estão desenhadas em `design/`. Próximo:
conectar o WhatsApp (Fase 4).

O projeto Supabase existe e as migrations `0001`–`0003` estão aplicadas lá; a
`0004` é drift capturado e ainda não foi aplicada. Subir para o Supabase real
expôs um bug que o Postgres local escondia — os grants por coluna do `0001` não
restringiam nada, porque o Supabase concede tudo por defeito e GRANT só soma. O
`0003` revoga essa herança; ver [doc 8](docs/08-fundacao.md).

Uma pendência que bloqueia: a identidade visual da Zona Nova — as telas usam
placeholder.